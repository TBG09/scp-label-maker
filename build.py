...
class RustBuilder:
    ...
    def build_target(self, target: str):
        ...
        elif self.config.use_cargo_xwin and "msvc" in target:
    ...
    def _get_binary_path(self, rust_target: str) -> Optional[Path]:
        ...
        if rust_target == "x86_64-unknown-linux-gnu":
            return Path("target/release/scp-label-maker")
    ...
    def package_builds(self):
        ...
        for target in self.config.targets:
            ...
            if not binary_path or not binary_path.exists():
                print(f"{Fore.YELLOW}⚠ Binary not found for {target}")
                continue
            
            target_dir = dist_dir / target
            target_dir.mkdir(exist_ok=True)
            
            dest = target_dir / binary_path.name
            subprocess.run(["cp", str(binary_path), str(dest)], check=True)
            print(f"{Fore.GREEN}✓ Packaged {target} -> {dest}")
